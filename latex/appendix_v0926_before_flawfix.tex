...
not a per-token bound and does not show that attention-only allocation is
optimal; a direct ablation of value-aware versus attention-only allocation
remains open.
...
\begin{itemize}
 ...
 \item In six decode-stability cells, the calibration-time family choice has p90 regret 1.00
...
The end-task audit is reported in Appendix~\ref{app:endtask}.  Beyond it, we
ran two development blocks of a natural task (LongBench-v2~\citep{bai2025longbenchv2},
forced choice).  Any candidate policy could improve on the best fixed policy on
only 3 of 52 and 2 of 52 questions, so the pre-registered sequential gates
stopped both blocks before the confirmation block was run.
...
H2O & $\lfloor BL/8\rfloor$ tokens at 8 bits; attention summed over all prefill queries & $\approx0.05$ \\
...
itself likely favors KVQuant.  H2O
...
\caption{Full end-task grid: mean score over 20 prompts per cell.  Bold marks the best method in each valid row.  $\ddagger$: uncompressed score below 0.9, excluded from every aggregate.  
KIVI, KVQuant, and H2O were measured only in the 8k run (Table~\ref{tab:endtask}).
}
\label{tab:app-grid}
...
% Estimate ledger commented out (2026-09-26); remove the comment environment to restore it.
\begin{comment}
\ifshowest
