#!/usr/bin/env python3
"""Authenticate and summarize the preregistered R8 k48 final-bracket cell.
...
question-agnostic multikey NIAH, B=2, k48/v4/h4, prompts 580--619, and exactly
...
Usage::
    ...
    .venv/bin/python \
      h0_measurement/bugs/9_sota_eviction_baselines/read_op2_k48.py \
      RESULT_K48.parquet [--csv SUMMARY.csv]

A valid artifact remains a successful read (exit status 0) when k48 is
...
NATIVE_CTX = 131_072
TASK = "niah_multikey"
PROMPTS = tuple(range(580, 620))
HALF_1 = tuple(range(580, 600))
HALF_2 = tuple(range(600, 620))
N_VALUES = 4
N_HOPS = 4
KEY_COUNT = 48
BUDGET = 2.0
...
def validate_frame(frame: pd.DataFrame, *, label: str = "parquet") -> int:
    ...
    if set(frame.prompt_idx.tolist()) != set(PROMPTS):
        _fail(f"{label} must contain exactly prompts 580..619")
...
def analyze_frame(frame: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "fp_580_599": float(first.fp.mean()),
        "uniform_580_599": float(first.uniform.mean()),
        "uniform_first_ok_580_599": float(first_ok.loc[list(HALF_1)].uniform.mean()),
        "fp_600_619": float(second.fp.mean()),
        "uniform_600_619": float(second.uniform.mean()),
        "uniform_first_ok_600_619": float(first_ok.loc[list(HALF_2)].uniform.mean()),
...
def analyze_artifact(path: os.PathLike[str] | str) -> tuple[pd.DataFrame, bool]:
    """Authenticate the single frozen k48 input and summarize its gates."""
...
def print_summary(summary: pd.DataFrame, eligible: bool) -> None:
    print("# R8 operating-point k48 final bracket")
    ...
    print(
        ...
        f"580-599(FP/uniform)={_fmt(row.fp_580_599)}/{_fmt(row.uniform_580_599)}  "
        f"600-619(FP/uniform)={_fmt(row.fp_600_619)}/{_fmt(row.uniform_600_619)}"
    )
    print(
        f"  first_ok FP/uniform={_fmt(row.fp_first_ok)}/{_fmt(row.uniform_first_ok)}  "
        f"uniform halves={_fmt(row.uniform_first_ok_580_599)}/"
        f"{_fmt(row.uniform_first_ok_600_619)}"
    )
    print(
    ...
    print(f"selection: {'k=48' if eligible else 'no selection'}")


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("parquet", help="the frozen k48 fp+uniform parquet")
    ...
    try:
        if args.validate_only:
            ...
            print("validated 1 k48 final-bracket artifact")
            return 0
