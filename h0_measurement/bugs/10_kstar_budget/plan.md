...
**Outcome (2026-09-24):** source-sealed Slurm array **985183** completed and authenticated. Q1, Q2, and Q4 passed; Q3 failed because Llama K*-proportional movement was 0.002319 against the frozen 0.05 minimum. The strict decision is **stop_r9_qualification**. No advance lock exists, and the development and confirmation partitions are closed.
...
## 5. Qualification: completed as job 985183
...
## 6. Conditional development and confirmation - closed

No qualification lock was written because Q3 failed. The specifications below
are retained as the pre-output record, but neither partition may run. A future
budget study must use a new protocol and fresh prompts; it cannot treat job
985183 as a development screen.
...
## 9. Authenticated outcome and disposition

| gate | observed | outcome |
|---|---:|---|
| Q1 G=1 reduction and budgets | max absolute difference 1.954e-14; exact K* match | pass |
| Q2 Llama stability | Spearman 0.751734 | pass |
| Q3 Llama redistribution | 0.002319 | **fail** |
| Q4 upper-bound resolution | 0/256 K*=k0 | pass |

The K* profile is stable but nearly common mode: Llama calibration K* has mean
2948.82, SD 17.53, and range 2867-2984 around k0=3059. Exact-total projection
moves only 1,816 of 783,104 slots one way. The failure is not caused by
rounding or bounds.

Post-result diagnosis also rejects the scalar's direction for this purpose.
K* uses a per-unit threshold relative to that unit's E_full. On Llama,
Spearman(K*, E_full) is -0.8009; projected donors contain 84.96% of E_full
mass and receivers 11.44%. K*-prop and shrink lose all six held-out Llama cells
and all six held-out Qwen cells. These held-out rows are descriptive and did
not change the frozen stop.

R9 development will not be submitted. Detailed authentication, tables, and
interpretation are in report.md and qualification_985183_summary.csv.

The immediate roadmap is:

1. reconcile the paper and proposal with R3-R9 under R1;
2. run R10's tier-set/3-bit-base work;
3. revisit budget allocation only as a separately preregistered hypothesis.

If step 3 is later authorized, replace the own-baseline K* scalar with a
whole-policy exact-total objective that preserves absolute curve losses or
frozen marginal gains. Extend curves above k0 before deriving recipient counts,
freeze the handling of nonmonotonicity and the optimizer, and use fresh prompts.
Do not lower Q3 or amplify this K* profile after seeing job 985183.
