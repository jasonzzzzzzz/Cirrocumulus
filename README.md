...
│   └── run_h1.py                 regenerates fig1 + fig4 and prints the τ table
│
...
- **H1 code** was three scripts (`h1_sim.py`, `h1_robust.py`, `h1_tau.py`) built on
  the pre-audit cost model. They are deleted. `h1_simulation/run_h1.py` replaces
  them and uses the corrected `sievelib` core, so the figures and the H0 code now
...
- **Figures.** `fig2_alloc.png` (the allocation staircase) was generated under the
  superseded relative-cost model and is deleted. `fig1`, `fig3`, `fig4` are current
