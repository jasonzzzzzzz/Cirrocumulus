#!/usr/bin/env python3
"""Authenticate and summarize the second R8 operating-point screen.

This reader is deliberately specific to the preregistered screen: Llama-3.1-8B
at 32K, question-agnostic multikey NIAH, B=2, prompts 500--539, and either 24
or 32 keys.  Each input is one ordinary R8 accuracy parquet accompanied by the
JSON sidecar written by :mod:`h0_measurement.run_r8`.

Usage::

    .venv/bin/python \
      h0_measurement/bugs/9_sota_eviction_baselines/read_op2.py \
      RESULT_K24.parquet RESULT_K32.parquet [--csv SUMMARY.csv]

The reporting path requires and authenticates both frozen cells before computing
any outcome. ``--validate-only`` exists for the workflow's per-job completion
guard and does not reveal scores. A valid paired experiment that has no eligible
operating point is still a successful read (exit status 0). Contract or
provenance failures use exit status 2.
"""
...
NATIVE_CTX = 131_072
TASK = "niah_multikey"
PROMPTS = tuple(range(500, 540))
HALF_1 = tuple(range(500, 520))
HALF_2 = tuple(range(520, 540))
N_VALUES = 4
N_HOPS = 4
KEY_COUNTS = (24, 32)
BUDGET = 2.0
...
REQUIRED_COLUMNS = (
    ...
    "max_new_tokens", "reached_max_new", "gen_len", "bits_per_token",
...
SINGLETON_COLUMNS = (
    ...
    "n_values", "n_hops", "max_new_tokens", "n_expected", "question_agnostic",
    "window", "observed_queries", "allocator_budget_rule", "maxb", "corpus_sha",
...
def validate_frame(frame: pd.DataFrame, *, label: str = "parquet") -> int:
    ...
    for column in ("ctx", "native_ctx", "prompt_idx", "n_keys", "n_values",
                   "n_hops", "max_new_tokens", "gen_len", "window", "maxb",
    ...
    expected_singletons = {
        ...
        "n_hops": N_HOPS, "max_new_tokens": MAX_NEW_TOKENS, "n_expected": 1,
    ...
    if n_keys not in KEY_COUNTS:
        _fail(f"{label}.n_keys={n_keys}; expected one of {list(KEY_COUNTS)}")
    ...
    if set(frame.prompt_idx.tolist()) != set(PROMPTS):
        _fail(f"{label} must contain exactly prompts 500..539")
    ...
    source = frame.groupby("prompt_idx")[["corpus_doc", "corpus_offset"]].nunique(
        dropna=False)
    if source.ne(1).any().any():
        _fail(f"{label} corpus document/offset differs between arms")
...
def analyze_frame(frame: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "fp_500_519": float(first.fp.mean()),
        "uniform_500_519": float(first.uniform.mean()),
        "uniform_first_ok_500_519": float(first_ok.loc[list(HALF_1)].uniform.mean()),
        "fp_520_539": float(second.fp.mean()),
        "uniform_520_539": float(second.uniform.mean()),
        "uniform_first_ok_520_539": float(first_ok.loc[list(HALF_2)].uniform.mean()),
...
def analyze_artifacts(paths: Sequence[os.PathLike[str] | str]) -> tuple[pd.DataFrame, int | None]:
    """Authenticate all inputs, summarize each k, and select the smallest eligible k."""
    if not paths:
        _fail("at least one parquet is required")
    frames = [load_artifact(path) for path in paths]
    key_counts = [int(_only(frame, "n_keys")) for frame in frames]
    if len(frames) != 2 or set(key_counts) != set(KEY_COUNTS):
        _fail(f"reporting requires exactly one k24 and one k32 artifact; got {key_counts!r}")
    corpus_ids = {str(_only(frame, "corpus_sha")) for frame in frames}
    if len(corpus_ids) != 1:
        _fail(f"artifacts do not share one real corpus identity: {sorted(corpus_ids)!r}")
    source_maps = []
    for frame in frames:
        source_maps.append(
            frame[["prompt_idx", "corpus_doc", "corpus_offset", "corpus_spliced"]]
            .drop_duplicates().sort_values("prompt_idx").reset_index(drop=True))
    if not source_maps[0].equals(source_maps[1]):
        _fail("k24 and k32 artifacts do not share exact prompt source provenance")

    summary = pd.DataFrame(analyze_frame(frame) for frame in frames)
    summary = summary.sort_values("n_keys").reset_index(drop=True)
    k24 = summary.loc[summary.n_keys == 24].iloc[0]
    k24_fp_stop = not (bool(k24.gate_fp_ge_095) and
                       bool(k24.gate_no_incomplete_capped_fp))
    eligible = summary.loc[summary.eligible, "n_keys"].astype(int).tolist()
    selected = None if k24_fp_stop else (min(eligible) if eligible else None)
    summary["k24_fp_stop"] = k24_fp_stop
    summary["selected"] = summary.n_keys.eq(selected) if selected is not None else False
...
def print_summary(summary: pd.DataFrame, selected: int | None) -> None:
    print("# R8 operating-point screen v2")
    ...
    for row in summary.itertuples(index=False):
        print(
            ...
            f"500-519(FP/uniform)={_fmt(row.fp_500_519)}/{_fmt(row.uniform_500_519)}  "
            f"520-539(FP/uniform)={_fmt(row.fp_520_539)}/{_fmt(row.uniform_520_539)}"
        )
        print(
            f"  first_ok FP/uniform={_fmt(row.fp_first_ok)}/{_fmt(row.uniform_first_ok)}  "
            f"uniform halves={_fmt(row.uniform_first_ok_500_519)}/"
            f"{_fmt(row.uniform_first_ok_520_539)}"
        )
        print(
    ...
    if bool(summary.k24_fp_stop.iloc[0]):
        print("selection: no selection (k24 FP/cap gate failed; stop higher-key screening)")
    else:
        print(f"selection: {'k=' + str(selected) if selected is not None else 'no selection'}")


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("parquets", nargs="+", help="R8 fp+uniform parquet(s)")
    parser.add_argument("--csv", help="optional path for the per-k summary CSV")
    ...
    try:
        if args.validate_only:
            ...
            for path in args.parquets:
                load_artifact(path)
            print(f"validated {len(args.parquets)} operating-point artifact(s)")
            return 0
        summary, selected = analyze_artifacts(args.parquets)
        print_summary(summary, selected)
        if args.csv:
