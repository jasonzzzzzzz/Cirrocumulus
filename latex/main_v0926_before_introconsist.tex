...
Compression is key-only with exact values, budgets match key code bits rather than total KV bytes. % and all quantization is simulated, so no memory or throughput claim is made.  
...
Within that scope, joint allocation is not uniformly useful.  Tier extinction orders when the interior lowers output error, context moves a model toward a corner, and allocations go stale faster than routes.  
On end tasks the output-error optimum chooses \emph{which tokens} to keep better than any evictor we test, but a model-matched dense quantizer does better still, and output distortion cannot make that choice.  
Its truncation failure points to the unit of allocation, which positional smoothing begins to repair.
