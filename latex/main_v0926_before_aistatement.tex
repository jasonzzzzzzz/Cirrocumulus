...
In this work, we used generative AI tools (an AI coding and writing assistant) for implementing and testing experiment code, including the KIVI and KVQuant-style baseline quantizers, the result readers, and the figure and table scripts; for preparing and submitting cluster jobs; for running statistical analyses on measured results; for searching and summarizing related literature and preparing bibliography entries; and for drafting and editing the text.  We have not used generative AI tools to generate, alter, or impute experimental data: every reported measurement is computed by the experiment code from model runs, and the few quantities still awaiting runs are marked as estimates and listed with their basis in Appendix~\ref{app:ledger}.  The research questions, the experimental design, and the pre-registered decision rules were set by the authors.  We have reviewed all AI-assisted work.  AI-written code was checked with unit tests, including a check of the RoPE inversion against a real model's cache, and with end-to-end runs before any GPU campaign.  Reported statistics were recomputed from the raw result files by the same readers that generate the tables.  Literature summaries were checked against the cited papers, and bibliography entries against arXiv or publisher pages.  We take responsibility for the final content of this work, including text, claims, or artifacts produced with the aid of generative AI.

\subsection*{Ethics statement}

This work measures compression of the key-value cache of publicly released language models on synthetic retrieval tasks built from public-domain books (PG-19).  It involves no human subjects, no personal or sensitive data, and no new model training.  The experiments used GPU time on a shared academic cluster.  To guard research integrity, each experiment's decision rules were written before its results were read, negative outcomes are reported alongside positive ones, and estimated values are distinguished from measurements throughout.
