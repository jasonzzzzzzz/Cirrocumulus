...
def quant_metrics(s: torch.Tensor, shat: dict[int, torch.Tensor], V: torch.Tensor,
    ...
    `practical_score` is a selection score available WITHOUT seeing the current
    query -- e.g. the previous decode step's attention weights, which is what
    H2O/SnapKV-style evictors actually use. The oracle eviction corner ranks by
    true sensitivity a_i||v_i-o||, which requires the very weights it is trying
    to avoid computing; it is therefore an upper bound on any real evictor and
    ...
    for B in budgets:
        ...
        if practical_score is not None:
            ...
            bp[torch.argsort(practical_score.double(), descending=True)[:m]] = maxb
            e_pr = exact_error(s, shat, V, bp)
            out[f"err_practical{B}"] = e_pr
            out[f"gain_practical{B}"] = e_pr / max(e_wf, 1e-12)
            out[f"gain_best_practical{B}"] = min(e_un, e_pr) / max(e_wf, 1e-12)
            out[f"in_band_practical{B}"] = float(
                out[f"gain_best_practical{B}"] >= BAND_MIN)
            out[f"oracle_evict_advantage{B}"] = e_pr / max(e_ev, 1e-12)
...
def head_metrics(s, shat, V, budgets=(1, 2, 3, 4), maxb=8, n_sink=4,
                 practical_score=None) -> dict:
    m = sensitivity_metrics(s, V, n_sink)
    if m and shat:
        m.update(quant_metrics(s, shat, V, budgets, maxb, practical_score))
    return m
