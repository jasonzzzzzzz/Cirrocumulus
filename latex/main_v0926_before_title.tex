...
\caption{\textbf{End-task audit}: mean RULER score over valid cells (FP $=0.999$). \emph{Full grid}: 36 cells, one run each. \emph{8k rerun}: all methods in one process on the same prompts (measured). Output error and effective bits ($B=2$) from the 8k rerun. \emph{Hard}: Llama-3.1-8B, 32k, 32-key, $B=2$. 
