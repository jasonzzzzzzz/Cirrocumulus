#!/usr/bin/env python3
"""CPU-only contract tests for the preregistered k48 follow-up reader."""
...
READER = ROOT / "h0_measurement/bugs/9_sota_eviction_baselines/read_op2_k48.py"
spec = importlib.util.spec_from_file_location("read_op2_k48", READER)
...
def fixture(uniform_successes=None):
    """Build a valid k48 cell; by default uniform succeeds 12 times per half."""
    if uniform_successes is None:
        uniform_successes = set(range(580, 592)) | set(range(600, 612))
    rows = []
    common = dict(
        model=RK.MODEL, model_id=RK.MODEL_ID, ctx=RK.CTX,
        native_ctx=RK.NATIVE_CTX, task=RK.TASK, n_keys=48,
        n_values=4, n_hops=4, task_n_needles=48, max_new_tokens=24,
    ...
    for prompt in RK.PROMPTS:
        depths = np.linspace(.05, .95, 48).round(4).tolist()
        rank = (prompt - RK.PROMPTS[0]) % 48
        target = dict(
...
def sidecar(path, frame):
    return dict(
        ...
        prompt_offset=580, window=32, observation_queries=[32],
...
def write_artifact(directory, frame, name="r8_llama31-8b_32768_k48_v4_h4.parquet"):
    path = directory / name
...
def test_metrics_and_gates():
    print("\n[k48 reader] frozen outcomes and gates")
    frame = fixture()
    ...
    check("both frozen halves are reported", row["uniform_580_599"] == .6 and
          row["uniform_600_619"] == .6)
    ...
    check("well-powered k48 cell is eligible",
    ...
    unstable = fixture(set(range(580, 598)) | set(range(600, 606)))
    ...
    too_easy = fixture(set(range(580, 596)) | set(range(600, 616)))
    ...
    capped = frame.copy()
    index = capped.index[(capped.arm == "fp") & (capped.prompt_idx == 580)][0]
...
def test_files_sidecar_and_exit_codes():
    print("\n[k48 reader] artifact, sidecar, CSV, and exit semantics")
    with tempfile.TemporaryDirectory() as tmp_name:
        ...
        check("CLI writes one-row CSV and exits zero",
              status == 0 and len(saved) == 1 and int(saved.n_keys.iloc[0]) == 48)
        with contextlib.redirect_stdout(io.StringIO()):
        ...
        no_path = write_artifact(
            no_dir,
            fixture(set(range(580, 599)) | set(range(600, 619))),
        )
        with contextlib.redirect_stdout(io.StringIO()):
...
def test_row_contract_rejections():
    print("\n[k48 reader] strict row provenance")
    frame = fixture()
    ...
    wrong_k = frame.copy()
    wrong_k["n_keys"] = 40
    rejects("only k48 is accepted", lambda: RK.validate_frame(wrong_k),
            "expected exactly 48")

    wrong_prompts = frame.copy()
    wrong_prompts.loc[wrong_prompts.prompt_idx == 619, "prompt_idx"] = 579
    rejects("only prompts 580..619 are accepted",
            lambda: RK.validate_frame(wrong_prompts), "prompts 580..619")
    ...
    duplicate_doc.loc[duplicate_doc.prompt_idx == 619, "corpus_doc"] = "book_38.txt"
    ...
    source_drift.loc[(source_drift.prompt_idx == 580) &
    ...
    token_drift.loc[(token_drift.prompt_idx == 580) &
    ...
    target_drift.loc[(target_drift.prompt_idx == 580) &
    ...
    bad_depth.loc[bad_depth.prompt_idx == 580, "target_needle_depth"] = .5
    ...
    index = vector_drift.index[(vector_drift.prompt_idx == 580) &
